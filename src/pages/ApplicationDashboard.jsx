...
function Content() {
  ...
  // The candidacy picker is always shown, but the application checklist only
  // applies to approved candidates — pending applicants (and members who've
  // merely opened the page to declare a run) get just the picker, mirroring the
  // pre-merge behavior where "My Application" was gated to approved candidates.
  const showChecklist =
    profile?.is_candidate_application && profile?.status !== 'pending'
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <div className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        {/* Candidacy declaration — which office you're on the ballot for. Kept
            as its own section because it writes to the election_cycles /
            election_candidates path, independent of the applicants checklist
            below. */}
        <CandidacySection />
...
// ---------------------------------------------------------------------------
// Candidacy declaration — self-service position picker for the open election
// cycle. Merged in from the former standalone "My Candidacy" page; it manages
// its own load/save because it talks to the my_candidacy / elected_positions
// path rather than the applicants checklist above.
// ---------------------------------------------------------------------------
function CandidacySection() {
  ...
  async function save() {
    ...
    // the cached auth profile so the application checklist unlocks immediately
    // rather than after the next auth event or reload.
    refreshProfile()
  }
