...
export default function Dashboard() {
  ...
  if (loading) {
    return <CenteredState icon={<Loader2 className="h-8 w-8 animate-spin" />} text="Loading…" />
  }
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-sm text-gray-500">
                  {formatDate(m.date)}
                  {m.agenda ? ` · ${m.agenda.slice(0, 60)}${m.agenda.length > 60 ? '…' : ''}` : ''}
                </p>
              </div>
              <button
                onClick={() => onToggle(m)}
...
function CenteredState({ icon, text, sub, action }) {
  return (
    <div className="min-h-screen bg-gray-50">
      <Navbar />
